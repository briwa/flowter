 ...
 */
export default {
  mixins: [FlowterFlowchartPropsMixin],
  computed: {
    /**
     ...
     */
    renderedNodes (): Record<string, GraphNodeDetails> {
     ...
     */
    allBounds (): Bounds {
      const { x, y } = this.nodeLists.reduce((bounds, row) => {
     ...
     */
    orderedNodes (): { dict: Record<string, OrderedNode>, maxIndex: number } {
      return this.edges.reduce((result, edge) => {
        const { dict } = result
  ...
  methods: {
     ...
     */
    getRenderedEdge (fromNodeDetails: GraphNodeDetails, toNodeDetails: GraphNodeDetails): RenderedEdge {
     ...
     */
    shapeNode (id: string, node: GraphNode): RenderedGraphNode {
     ...
     */
    getEdgeDirection (from: GraphNodeDetails, to: GraphNodeDetails): Direction {
      switch (this.mode) {
        case Mode.VERTICAL: {
     ...
     */
    getEdgeSide (from: GraphNodeDetails, to: GraphNodeDetails) {
      const {
        node: fromNode,
        row: fromRow
      } = from
     ...
     */
    getNodeDirections (node: RenderedGraphNode): Record<Direction, { x: number, y: number }> {
      return {
     ...
     */
    shapeNodesVertically (nodeList: NodeRow[], maxWidth: number) {
     ...
     * if specified from [[nodes]].
     */
    shapeNodesHorizontally (nodeList: NodeRow[], maxHeight: number) {
     ...
     */
    getBounds (bounds: Bounds, node: RenderedGraphNode): Bounds {
